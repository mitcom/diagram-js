...
function LassoTool(eventBus, canvas, dragging, elementRegistry, selection) {
  ...
  // Select elements
  eventBus.on('selectionBox.end', function(event) {
  ...
  function startDragging(event) {
    ...
    dragging.activate(event, 'selectionBox', {
      autoActivate: true,
      data: {
        context: {}
      }
  ...
  eventBus.on('selectionBox.start', function(event) {
  ...
  eventBus.on('selectionBox.move', function(event) {
  ...
  eventBus.on('selectionBox.cleanup', function(event) {
  ...
  var visuals = {
    create: function(context) {
      ...
      frame = context.frame = Snap.create('rect', {
        id: 'djs-selection-box',
        class: 'djs-select-box-overlay',
        width:  1,
        height: 1,
        x: 0,
        y: 0
